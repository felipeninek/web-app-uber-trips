...
def filtrar_por_hora(df, hora_inicio, hora_fim):
    return df[(df['Data'].dt.time >= pd.to_datetime(hora_inicio).time()) & 
              (df['Data'].dt.time <= pd.to_datetime(hora_fim).time())]

def filtrar_por_dia_semana(df, dia_semana):
    return df[df['Dia da Semana'] == dia_semana]

def filtrar_por_semana_mes(df, semana_mes):
    return df[df['Semana do Mês'] == semana_mes]

def filtrar_por_mes_ano(df, mes_ano):
    return df[df['Mês'] == mes_ano]
...
def aumente_seus_ganhos():
    ...
    df['Preço (R$)'] = df['Preço (R$)'].str.replace(',', '.').astype('float64')
    df['Latitude (Origem)'] = df['Latitude (Origem)'].str.replace(',', '.').astype('float64')
    df['Longitude (Origem)'] = df['Longitude (Origem)'].str.replace(',', '.').astype('float64')
    df['Latitude (Destino)'] = df['Latitude (Destino)'].str.replace(',', '.').astype('float64')
    df['Longitude (Destino)'] = df['Longitude (Destino)'].str.replace(',', '.').astype('float64')
    ...
    media_preco = df_filtrado['Preço (R$)'].mean()

    # Adicionar uma coluna para a cor do marcador
    df_filtrado['Cor'] = df_filtrado['Preço (R$)'].apply(lambda x: 'green' if x > media_preco else 'red')
    ...
    for _, row in df_filtrado.iterrows():
        popup_text = f"Preço: R${row['Preço (R$)']:.2f}<br>Média: R${media_preco:.2f}"

        if pd.notna(row['Latitude (Origem)']) and pd.notna(row['Longitude (Origem)']):
            folium.Marker(
                location=[row['Latitude (Origem)'], row['Longitude (Origem)']],
                popup=popup_text,
                icon=folium.Icon(icon='circle', color=row['Cor'])
            ).add_to(mapa)
            
        if pd.notna(row['Latitude (Destino)']) and pd.notna(row['Longitude (Destino)']):
            folium.Marker(
                location=[row['Latitude (Destino)'], row['Longitude (Destino)']],
                popup=popup_text,
                icon=folium.Icon(icon='circle', color=row['Cor'])
            ).add_to(mapa)
